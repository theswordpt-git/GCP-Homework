#moved the firewall rules to 6-firewall.tf
...
#Chewbacca: Instance template—MIG starter pack.
resource "google_compute_instance_template" "chewbacca_template" {
  name         = "chewbacca-template"
  machine_type = "e2-medium"  #instead of n2-standard-2, ZONE_RESOURCE_POOL_EXHAUSTED errors

  disk {
    boot        = true
    auto_delete = true
    #image = "centos-cloud/centos-stream-10"  doesn't work
    source_image = "centos-cloud/centos-stream-10" #does ughhhhhh


    #size  = 100
    #size doesn't work here either
    
    
    #hitting ZONE_RESOURCE_POOL_EXHAUSTED, commented out	
    #disk_size_gb = 100 #works, ughhhhhhhhh

  }

  network_interface {
    network    = google_compute_network.chewbacca_vpc.id
    subnetwork = google_compute_subnetwork.chewbacca_subnet.id
...
#Chewbacca: The fleet deploys—MIG definition.
resource "google_compute_instance_group_manager" "chewbacca_mig" {
  name = "chewbacca-mig"
  zone = var.zone
  version {
    instance_template = google_compute_instance_template.chewbacca_template.id
  }
